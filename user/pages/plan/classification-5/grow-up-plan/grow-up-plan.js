...
Page({
  ...
  data: {
    planType: [
      ...
      ['篮球', '足球', '乒乓球','滑雪', '游泳', '网球', '排球', '滑冰', '跑步', '散打', '搏击', '高尔夫', '马术', '没有可自填']
    ],
    planTypeIndex: [],
    targetArray: ['提高个人品味', '提高个人涵养', '个人爱好', '工作需要', '没有可自填'],
    inputType: '',
    inputInfo: {},
    inputValue: '',
    targetIndex: -1,
  ...
  changeColumnc(e) {
    ...
    if(column == 0 && value == 0) {
      data.planType[1] = ['篮球', '足球', '乒乓球','滑雪', '游泳', '网球', 
      '排球', '滑冰', '跑步', '散打', '搏击', '高尔夫', '马术', '没有可自填']
    }
    if(column == 0 && value == 1) {
      data.planType[1] = ['五子棋', '象棋', '国际象棋','围棋', '没有可自填']
    }
    if(column == 0 && value == 2) {
      data.planType[1] = ['芭蕾', '民族', '现代', '民间', '踢踏', '爵士', '街舞', '交谊舞', '没有可自填']
    }
    if(column == 0 && value == 3) {
      data.planType[1] = ['钢琴', '小提琴', '大提琴','手风琴', '吉他', '打击乐器', '铜管乐器', '没有可自填']
    }
    if(column == 0 && value == 4) {
      data.planType[1] = ['美声', '民族', '通俗', '唱歌达人', '没有可自填']
    }
    if(column == 0 && value == 5) {
      data.planType[1] = ['运势规划', '咖啡品鉴', '面包烘培', '红酒品鉴','健身瘦身', '户外旅游', 
      '钓鱼', '阅读', '插画师', '配音师', '沙画师', '直播网红', '健康养生']
    }
    this.setData(data)
  },
  bindTargetChange(e) {
